...
function create_cubemobile()
{
  ...
  _groupId=$1
  _webportal_mac=($(mysql $db_name -e "select mac_address from mobile_mac where is_login='Y' and login_time > DATE_FORMAT(DATE_ADD(now(),INTERVAL -$interval MINUTE),'%Y-%m-%d %H:%i:%s')" |egrep '^([[:xdigit:]]{2}[:.-]?){5}[[:xdigit:]]{2}$'))
...
while true; do
  create_cubemobile $CUBEMOBILE_ID $CUBEMOBILE
  rotate_log
  sleep 1
done
