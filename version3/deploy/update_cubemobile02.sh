...
# DB에 등록된 맥주소의 최초 로그인 타임에 현재 조회한 시간 기준에 따른 대한 시간 간격 (단위: 분)
interval=60

LOG=/var/log/batch_update.log

function update_cubemobile()
{
          ...
          if [ "$is_exist_in_ise" == "$webportal_mac_address" ]; then
            ...
            curl -s --insecure  \
                  ...
                  --data '
                        ...
                        }'|egrep "HTTP|title" | awk '{ print strftime("%Y-%m-%d %T") " '$2' Try updating endpoint('$webportal_mac_address') to ise('$2').. the result: ", $0; fflush() }' >> $LOG #|grep "title"  |awk -F" : " '{print  $2}' | awk '{ print strftime("%Y-%m-%d %T"), $0; fflush() }' >> $LOG
...
while true; do
  update_cubemobile $CUBEMOBILE_ID $CUBEMOBILE
  rotate_log
  sleep 1
done