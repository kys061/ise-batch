...
function compare_cubevdi_endpoint()
{
    ...
    _groupId=$1
    webportal_mac=($(mysql $db_name -e "select mac_address from internal_assets_mac where as_cls_cd = 1 and as_ident_cd = 1 and as_detail_cd = 5 and is_login = 'Y'" |egrep '^([[:xdigit:]]{2}[:.-]?){5}[[:xdigit:]]{2}$'))
